...
export async function addPassword(request: Request, response: Response): Promise<void> {
  ...
  if (!userId) {
   ...
   const existingUser = await Password.findOne({ name, usernameOrUserId, websiteLink });
    if (existingUser) {
  ...
  try {
    ...
    const newPassword: IPassword = new Password({
      ...
      tag: tag,
      userId: request.user?.userId, // Associate the password entry with the authenticated user
    });
...
export async function updatePassword(req: Request, res: Response): Promise<void> {
  ...
  try {
    ...
    if (passwordEntry.userId.toString() !== req.user.userId) {
       res.status(403).json({ message: 'Not authorized to update this password entry' });
       return
    }
    ...
    if (usernameOrUserId !== undefined) updateFields.usernameOrUserId = usernameOrUserId;
...
export async function deletePassword(req: Request, res: Response): Promise<void> {
    const { id } = req.params; // Get the password entry ID from the URL parameters
  ...
  try {
    ...
    // Ensure the authenticated user owns this password entry
    if (passwordEntry.userId.toString() !== req.user.userId) {
       res.status(403).json({ message: 'Not authorized to delete this password entry' });
       return
    }
